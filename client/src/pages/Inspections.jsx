...
import { 
  ClipboardList, ShieldCheck, Calendar, AlertCircle, Plus, ChevronRight, Info, MoreVertical 
} from 'lucide-react';
...
export const Inspections = () => {
  ...
  const completedCount = inspections.filter(i => i.date && i.date !== '—').length;
  ...
  const scheduledPct = totalCount ? Math.round((scheduledCount / totalCount) * 100) : 0;
  ...
  const formattedInspections = inspections.map((i, idx) => {
    ...
    return {
      ...
      date: hasBeenCompleted ? i.date : (i.next_inspection_due ? new Date(i.next_inspection_due).toLocaleDateString('en-GB') : '—'),
      countdown: hasBeenCompleted ? '' : (countdownInfo?.text || ''),
      countdownColor: isOverdue ? 'text-status-danger font-bold' : 'text-[#4A5568]',
      status: statusLabel,
      action: hasBeenCompleted ? 'View Report' : 'View Details',
      color: hasBeenCompleted ? 'bg-status-success-bg text-status-success' : isOverdue ? 'bg-status-danger-bg text-status-danger' : 'bg-status-info-bg text-status-info',
  ...
  // Sidebar collections
  const upcomingList = formattedInspections.filter(i => i.status === 'Scheduled');
  const overdueList = formattedInspections.filter(i => i.status === 'Overdue');
      ...
      <div className="py-6 flex flex-col gap-6 max-w-[1440px] mx-auto px-8 animate-pulse">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-3 flex flex-col gap-6">
            ...
            <div className="h-[150px] bg-gray-100/60 rounded-2xl" />
          </div>
        </div>
      </div>
    );
                    ...
                    paginatedInspections.map((row, idx) => {
                      ...
                      return (
                        <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="py-3 px-2 font-semibold text-gray-500">{row.type}</td>
                          <td className="py-3 px-2 font-semibold text-gray-500 leading-tight">{row.inspector}</td>
                          <td className="py-3 px-2">
                            <div className="flex flex-col leading-tight">
                              <span className="font-semibold text-[#1A1A1A]">{row.date}</span>
                              {row.countdown && <span className={`text-2xs mt-0.5 leading-none ${row.countdownColor}`}>{row.countdown}</span>}
                            </div>
                          </td>
                          <td className="py-3 px-2">
        ...
        <div className="lg:col-span-3 flex flex-col gap-6 select-none">
          ...
          <div className="bg-white border border-card-border rounded-card p-5 shadow-xs flex flex-col gap-4">
            <div className="flex justify-between items-baseline border-b border-gray-50 pb-2">
              <h3 className="text-sm-portal font-bold text-brand-primary uppercase tracking-wider">Upcoming Inspections</h3>
            ...
            <div className="flex flex-col gap-3">
              {loading ? (
                <div className="text-xs-portal text-gray-400 font-semibold py-2">Loading...</div>
              ) : upcomingList.length === 0 ? (
                <div className="text-xs-portal text-gray-400 font-semibold py-2">No upcoming inspections</div>
              ) : (
                upcomingList.slice(0, 3).map((item, idx) => (
                  <div key={idx} className={`flex items-center justify-between gap-3 text-xs-portal ${idx > 0 ? 'border-t border-gray-50 pt-2.5' : ''}`}>
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-status-info-bg text-status-info flex items-center justify-center shrink-0">
                        <Calendar size={11} />
                      </div>
                      <div className="flex flex-col text-left">
                        <span className="font-bold text-brand-primary leading-tight">{item.item}</span>
                        <span className="text-2xs text-gray-400 leading-none mt-0.5 truncate max-w-[120px]">{item.property}</span>
                      </div>
                    </div>
                    <div className="flex flex-col items-end text-right leading-none shrink-0">
                      <span className="font-semibold text-brand-primary">{item.date}</span>
                      {item.countdown && <span className="text-2xs text-[#4A5568] font-bold mt-1">{item.countdown}</span>}
                    </div>
                  </div>
                ))
              )}
            ...
            <Button 
              variant="secondary" 
              size="sm" 
              className="w-full font-bold bg-white text-xs-portal mt-1"
              onClick={() => {}}
            >
              View All Scheduled
            </Button>
          ...
          <div className="bg-white border border-card-border rounded-card p-5 shadow-xs flex flex-col gap-4">
            <div className="flex justify-between items-baseline border-b border-gray-50 pb-2">
              <h3 className="text-sm-portal font-bold text-brand-primary uppercase tracking-wider">Overdue Inspections</h3>
              <button className="text-xs-portal font-bold text-status-info hover:underline cursor-pointer">
                View All
              </button>
            </div>
            ...
            <div className="flex flex-col gap-3">
              {loading ? (
                ...
                <div className="text-xs-portal text-gray-400 font-semibold py-2">No overdue inspections</div>
              ) : (
                overdueList.slice(0, 3).map((item, idx) => (
                  <div key={idx} className={`flex items-center justify-between gap-3 text-xs-portal ${idx > 0 ? 'border-t border-gray-50 pt-2.5' : ''}`}>
                    ...
                    <div className="flex flex-col items-end text-right leading-none shrink-0">
                      <span className="font-semibold text-brand-primary">{item.date}</span>
                      {item.countdown && <span className="text-2xs text-status-danger font-bold mt-1">{item.countdown}</span>}
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="bg-white border border-card-border rounded-card p-5 shadow-xs flex flex-col gap-4">
            <div className="flex gap-3.5 items-start">
              ...
              <div className="flex flex-col text-left">
                <h4 className="text-xs-portal font-bold text-brand-primary uppercase tracking-wider">Need to Schedule an Inspection?</h4>
