...
import { 
  FileText, Calendar, AlertCircle, ShieldCheck, Download, ChevronRight, Info, PlusCircle, MoreVertical 
} from 'lucide-react';
...
export const Certificates = () => {
  ...
  // Dynamically derive certificates list from real properties compliance states
  ...
  properties.forEach(p => {
    ...
    if (p.gasCompliance) {
      ...
      certificatesData.push({
        ...
        countdownColor: isExpired ? 'text-status-danger font-bold' : 'text-[#4A5568]',
    ...
    if (p.eicrCompliance) {
      ...
      certificatesData.push({
        item: 'Electrical Installation Certificate (EICR)',
        ref: `Ref: EICR-${p.id}`,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        ...
                        <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="py-3 px-2 font-medium text-gray-400">
                            <div className="flex flex-col leading-tight">
                              ...
                              <span className={`text-2xs mt-0.5 leading-none ${row.countdownColor}`}>{row.countdown}</span>
                            </div>
                          </td>
                          <td className="py-3 px-2">
        ...
        <div className="lg:col-span-3 flex flex-col gap-6 select-none">
          ...
          <div className="bg-white border border-card-border rounded-card p-5 shadow-xs flex flex-col gap-4">
            <h3 className="text-sm-portal font-bold text-brand-primary uppercase tracking-wider border-b border-gray-50 pb-2">
              Expiry Warnings
            </h3>
            ...
            <div className="flex flex-col gap-3">
              {loading ? (
                ...
                <div className="text-xs-portal text-gray-400 font-semibold py-2">No certificates expired or expiring soon</div>
              ) : (
                certificatesData.filter(c => c.status === 'Expired').slice(0, 3).map((item, idx) => (
                  <div key={idx} className={`flex items-center justify-between gap-3 text-xs-portal ${idx > 0 ? 'border-t border-gray-50 pt-2.5' : ''}`}>
                    ...
                    <StatusPill status="expired" size="sm" rounded="sm" showIcon={false} />
                  </div>
                ))
              )}
            </div>
          ...
          <div className="bg-white border border-card-border rounded-card p-5 shadow-xs flex flex-col gap-4 text-left">
            <div className="flex gap-3.5 items-start">
              ...
              <div className="flex flex-col">
                <h4 className="text-xs-portal font-bold text-[#1A1A1A] uppercase tracking-wider">Need a New Certificate?</h4>
                <p className="text-2xs text-gray-400 font-semibold mt-2.5 leading-relaxed">
                  Request a new compliance inspection and certification for your rental property.
                </p>
              </div>
            </div>
