...
export const Dropdown = ({
  ...
  const sizeMap = {
    ...
    lg: {
      trigger: "h-12 text-base px-4  gap-2.5",
      menu: "text-base",
      icon: 18,
    },
  };
  ...
  const variantMap = {
    ...
    ghost: `bg-transparent border-0 hover:bg-surface-light rounded-card`,
  };
  ...
  return (
    ...
    >
      ...
      {label && (
        <label
          htmlFor={id}
          className="text-xs font-semibold text-status-muted uppercase tracking-wide"
        >
          {label}
        </label>
      )}
      ...
      >
        ...
        <span className="flex items-center gap-2 min-w-0 flex-1">
          {LeadIcon && (
            <LeadIcon size={sz.icon} className="text-gray-400 shrink-0" />
          )}
          <span
            className={`truncate font-medium ${displayLabel ? "text-text-primary" : "text-gray-400"}`}
          >
        ...
        <span className="flex items-center gap-1 shrink-0 ml-1">
          {clearable && selectedValues.length > 0 && !disabled && (
            <span
              ...
              className="p-0.5 rounded hover:bg-surface-hover text-gray-400 hover:text-status-muted transition-colors"
            >
              <X size={sz.icon - 2} />
            </span>
          )}
      ...
      {/* Error text */}
      {error && (
        <p className="text-xs font-semibold text-status-danger mt-0.5">
          {error}
        </p>
      )}
        ...
        >
          ...
          {searchable && (
            <div className="px-2 pt-2 pb-1 border-b border-border-color/50">
              <div className="flex items-center gap-2 bg-app-bg rounded-card px-2.5 py-1.5">
                ...
                <input
                  ...
                  onKeyDown={handleKeyDown}
                  placeholder="Search…"
                  className="flex-1 bg-transparent text-xs text-text-primary placeholder:text-gray-400 focus:outline-none min-w-0"
                />
                {search && (
                  <button
                    onClick={() => setSearch("")}
                    className="text-gray-400 hover:text-status-muted"
                  >
                    <X size={11} />
                  </button>
                )}
            ...
            {filteredOptions.length === 0 ? (
              ...
              (() => {
                ...
                return filteredOptions.map((opt, idx) => {
                  ...
                  // Group header
                  const groupHeader =
                    opt.group && !groupRendered[opt.group]
                      ? (() => {
                          groupRendered[opt.group] = true;
                          return opt.group;
                        })()
                      : null;
                  ...
                  return (
                    <React.Fragment key={opt.value ?? idx}>
                      ...
                      <li
                        ...
                        onMouseEnter={() =>
                          !opt.disabled && setFocusedIdx(currentEnabledIdx)
                        }
                        className={`
                          ...
                          ${isFocused && !isSelected && !opt.disabled ? "bg-surface-light text-text-primary" : ""}
                          ${!isSelected && !isFocused ? "text-text-primary" : ""}
                        `}
                      >
                        {/* Left icon */}
