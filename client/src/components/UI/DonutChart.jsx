...
export const DonutChart = ({ segments = [], total, centerLabel, centerSubLabel, size = 80 }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute text-center leading-none flex flex-col items-center justify-center">
        <span className="text-xs-portal font-black text-brand-primary">{centerLabel ?? totalVal}</span>
        <span className="text-[5.5px] text-gray-400 font-extrabold uppercase mt-0.5 tracking-wider">{centerSubLabel ?? 'Total'}</span>
      </div>
    </div>
  );
};
