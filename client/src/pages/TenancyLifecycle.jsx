...
import { 
  Users, ArrowUpRight, CheckCircle2, ChevronRight, Info, Calendar, Settings, MoreVertical, RefreshCw, LogOut, Home, FileText, Check 
} from 'lucide-react';
...
export const TenancyLifecycle = () => {
  ...
  const pendingMoveInCount = tenancies.filter(t => t.status === 'pending' || (t.start_date && new Date(t.start_date) > new Date())).length;
  const pendingMoveInPct = totalCount ? Math.round((pendingMoveInCount / totalCount) * 100) : 0;
  ...
  const renewalsPct = totalCount ? Math.round((renewalsCount / totalCount) * 100) : 0;

  // Move-outs (end date in current month or status is ended/closed)
  const moveOutsCount = tenancies.filter(t => t.status === 'ended' || t.status === 'closed').length;
  const moveOutsPct = totalCount ? Math.round((moveOutsCount / totalCount) * 100) : 0;
  ...
  return (
    <div className="py-6 flex flex-col gap-6 max-w-[1440px] mx-auto px-8 font-sans text-brand-primary font-medium">
      ...
      <div className="bg-white border border-card-border rounded-card p-5 shadow-xs flex flex-col gap-5 select-none">
        <h3 className="text-sm-portal font-bold text-brand-primary uppercase tracking-wider pb-3 border-b border-gray-50/50">
        ...
        <div className="flex flex-col md:flex-row items-stretch justify-between gap-6 py-4 relative">
          ...
          <div className="absolute left-[8%] right-[8%] top-[34px] h-0.5 bg-gray-100 hidden md:block z-0" />
          ...
          <div className="flex flex-col items-center text-center flex-1 relative z-10">
            ...
            <span className="text-xs-portal font-black text-brand-primary mt-3">1. Application</span>
            <p className="text-2xs text-gray-400 font-semibold mt-1 max-w-[120px] leading-relaxed">Tenant applies and is screened.</p>
            <span className="text-2xs font-extrabold text-brand-primary mt-2 block bg-gray-50 border border-gray-100 px-2 py-0.5 rounded-sm">
          ...
          <div className="flex flex-col items-center text-center flex-1 relative z-10">
            ...
            <span className="text-xs-portal font-black text-brand-primary mt-3">2. Move In</span>
            <p className="text-2xs text-gray-400 font-semibold mt-1 max-w-[120px] leading-relaxed">Tenancy agreement signed and tenant moves in.</p>
            <span className="text-2xs font-extrabold text-brand-primary mt-2 block bg-gray-50 border border-gray-100 px-2 py-0.5 rounded-sm">
          ...
          <div className="flex flex-col items-center text-center flex-1 relative z-10">
            ...
            <span className="text-xs-portal font-black text-status-info mt-3">3. Ongoing Tenancy</span>
            <p className="text-2xs text-gray-400 font-semibold mt-1 max-w-[120px] leading-relaxed">Tenancy is active and being managed.</p>
          ...
          {/* Step 4: Renewal */}
          <div className="flex flex-col items-center text-center flex-1 relative z-10">
            <div className="w-11 h-11 rounded-full bg-white border border-gray-200 text-gray-400 flex items-center justify-center">
              <RefreshCw size={15} />
            </div>
            <span className="text-xs-portal font-black text-gray-500 mt-3">4. Renewal</span>
            <p className="text-2xs text-gray-400 font-semibold mt-1 max-w-[120px] leading-relaxed">Tenancy renewal offered or agreement updated.</p>
            <span className="text-2xs font-extrabold text-gray-500 mt-2 block bg-gray-50 border border-gray-100 px-2 py-0.5 rounded-sm">
          ...
          <div className="flex flex-col items-center text-center flex-1 relative z-10">
            ...
            <span className="text-xs-portal font-black text-gray-500 mt-3">5. Move Out</span>
            <p className="text-2xs text-gray-400 font-semibold mt-1 max-w-[120px] leading-relaxed">Tenant moves out and tenancy is closed.</p>
            <span className="text-2xs font-extrabold text-gray-500 mt-2 block bg-gray-50 border border-gray-100 px-2 py-0.5 rounded-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          <div className="bg-white border border-card-border rounded-card p-5 shadow-xs flex flex-col justify-between overflow-hidden min-h-[300px]">
            <div className="flex justify-between items-center pb-3 border-b border-gray-50 select-none">
              <h3 className="text-sm-portal font-bold text-brand-primary uppercase tracking-wider">Upcoming Renewals</h3>
              <button className="text-xs-portal font-bold text-status-info hover:underline cursor-pointer">
                View All Renewals
              </button>
            </div>
                  ...
                  {loading || formattedRenewals.length === 0 ? (
                    ...
                    formattedRenewals.map((row, idx) => (
                      <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                        <td className="py-3 px-2 font-bold text-brand-primary">{row.tenancy}</td>
                        <td className="py-3 px-2 text-gray-500 font-semibold">{row.property}</td>
                        <td className="py-3 px-2 font-semibold text-gray-500">{row.tenant}</td>
                        <td className="py-3 px-2 font-semibold text-brand-primary">{row.date}</td>
                        <td className="py-3 px-2">
                          <span className={`px-2 py-0.5 text-2xs font-bold rounded-sm border select-none inline-block ${row.statusColor}`}>
                            {row.countdown}
                          </span>
                        </td>
                        <td className="py-3 px-2">
                          <div className="flex items-center justify-center gap-1">
                            <Button variant="secondary" className="!py-0.5 !px-2.5 text-2xs font-bold bg-white">Review Renewal</Button>
          ...
          {/* Bottom callout */}
          <div className="bg-white border border-card-border rounded-card p-5 shadow-xs flex gap-4 items-center text-left select-none">
            <div className="w-10 h-10 rounded-full bg-status-info-bg text-status-info flex items-center justify-center shrink-0">
              <Info size={16} />
            </div>
            <div className="flex flex-col">
              <h4 className="text-xs-portal font-bold text-brand-primary uppercase tracking-wider leading-none">Streamline Your Tenancy Management</h4>
              <p className="text-2xs text-gray-400 font-semibold mt-2.5 leading-none">Track every stage of the tenancy lifecycle and stay on top of key dates and actions.</p>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-6 select-none">
            ...
            <div className="flex items-center gap-6 mt-2">
              <DonutChart 
                segments={[
                  ...
                  { value: pendingMoveInCount, colorClass: 'stroke-status-success' },
              ...
              <div className="flex flex-col gap-1.5 text-2xs font-bold text-gray-500">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-brand-primary font-extrabold ml-1">{activeCount} ({activePct}%)</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-status-success shrink-0" />
                  <span>Pending Move Ins</span>
                  <span className="text-brand-primary font-extrabold ml-1">{pendingMoveInCount} ({pendingMoveInPct}%)</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-status-warning shrink-0" />
                  <span>Upcoming Renewals</span>
                  <span className="text-brand-primary font-extrabold ml-1">{renewalsCount} ({renewalsPct}%)</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-status-danger shrink-0" />
                  <span>Pending Move Outs</span>
                  <span className="text-brand-primary font-extrabold ml-1">{moveOutsCount} ({moveOutsPct}%)</span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white border border-card-border rounded-card p-5 shadow-xs flex flex-col gap-4">
            <div className="flex justify-between items-baseline border-b border-gray-50 pb-2">
              <h3 className="text-sm-portal font-bold text-brand-primary uppercase tracking-wider">Tasks & Actions</h3>
              <button className="text-xs-portal font-bold text-status-info hover:underline cursor-pointer">
                View All Tasks
              </button>
            </div>
            ...
            <div className="flex flex-col gap-3 mt-1">
              <div className="text-xs-portal text-gray-400 font-semibold py-2">No pending tasks or actions</div>
            </div>
          </div>
